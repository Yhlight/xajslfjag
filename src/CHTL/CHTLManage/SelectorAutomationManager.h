...
struct SelectorInfo {
    ...
    SelectorInfo() : type(SelectorType::CLASS_SELECTOR), isAutoGenerated(false) {}
    
    SelectorInfo(SelectorType t, const std::string& n, bool autoGen = false)
...
public:
     ...
     * 对于脚本：优先选择ID
     */
    void setReferencePriority(const std::string& context, SelectorType priority);
    
    /**
     * 获取引用优先级
     */
    SelectorType getReferencePriority(const std::string& context) const;

private:
    SelectorAutomationManager* m_automationManager;                // 自动化管理器
    std::unordered_map<std::string, SelectorType> m_priorities;    // 上下文优先级映射
     ...
     * 验证引用上下文
     */
    bool isValidReferenceContext(const std::string& context, size_t position);
};
