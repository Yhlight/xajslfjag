...
std::string SelectorAutomationManager::generateAutoClassName(int index) const {
    if (index < 0) {
        index = m_automationRules.initialClassIndex;
        while (m_usedClassNames.find(m_automationRules.classPrefix + std::to_string(index)) != m_usedClassNames.end()) {
            index++;
        }
    }
    return m_automationRules.classPrefix + std::to_string(index);
}

std::string SelectorAutomationManager::generateAutoIdName(int index) const {
    if (index < 0) {
        index = m_automationRules.initialIdIndex;
        while (m_usedIdNames.find(m_automationRules.idPrefix + std::to_string(index)) != m_usedIdNames.end()) {
            index++;
        }
    }
    return m_automationRules.idPrefix + std::to_string(index);
}

bool SelectorAutomationManager::hasAttribute(std::shared_ptr<ElementNode> element, const std::string& attributeName) const {
    if (!element) return false;
    
    // 检查元素是否已有指定属性
    if (attributeName == "class") {
        return !element->getClass().empty();
...
void SelectorAutomationManager::addAttribute(std::shared_ptr<ElementNode> element, const std::string& attributeName, const std::string& value) {
    if (!element) return;
    
    // 根据属性类型设置对应的值
    if (attributeName == "class") {
        // 如果已有class，添加新的class；否则设置class
        if (!element->getClass().empty()) {
            element->addClass(value);
        } else {
            element->setClass(value);
        }
        m_usedClassNames.insert(value);
    } else if (attributeName == "id") {
        element->setId(value);
        m_usedIdNames.insert(value);
    }
}

void SelectorAutomationManager::loadAutomationRulesFromConfig(const std::unordered_map<std::string, std::string>& configSettings) {
    m_automationRules.applyConfigSettings(configSettings);
}

void SelectorAutomationManager::processEnhancedSelectorAutomation(std::shared_ptr<ElementNode> element, const std::vector<SelectorInfo>& enhancedSelectors) {
    if (!element || enhancedSelectors.empty()) return;
    ...
    for (const auto& selector : enhancedSelectors) {
        if (selector.name.find("{{.") == 0) {
            // 类选择器: {{.box}}
            SelectorInfo classSelector;
            classSelector.type = AutomationSelectorType::CLASS_SELECTOR;
            classSelector.name = selector.name.substr(3, selector.name.length() - 5); // 去掉 {{. 和 }}
            classSelector.isAutoGenerated = false;
            classSelectors.push_back(classSelector);
        } else if (selector.name.find("{{#") == 0) {
            // ID选择器: {{#box}}
            SelectorInfo idSelector;
            idSelector.type = AutomationSelectorType::ID_SELECTOR;
            idSelector.name = selector.name.substr(3, selector.name.length() - 5); // 去掉 {{# 和 }}
            idSelector.isAutoGenerated = false;
            idSelectors.push_back(idSelector);
        }
    ...
    // 应用自动化规则
    if (!classSelectors.empty() && m_automationRules.enableScriptAutoClass && !hasAttribute(element, "class")) {
        const auto& firstClass = classSelectors[0];
        addAttribute(element, "class", firstClass.name);
    }
    
    if (!idSelectors.empty() && m_automationRules.enableScriptAutoId && !hasAttribute(element, "id")) {
        const auto& firstId = idSelectors[0];
        addAttribute(element, "id", firstId.name);
    }
}

// ========== AutomationRule 扩展实现 ==========

void AutomationRule::applyConfigSettings(const std::unordered_map<std::string, std::string>& settings) {
    for (const auto& [key, value] : settings) {
        if (key == "DISABLE_STYLE_AUTO_ADD_CLASS") {
            enableStyleAutoClass = (value != "true");
        } else if (key == "DISABLE_STYLE_AUTO_ADD_ID") {
            enableStyleAutoId = (value != "true");
        } else if (key == "DISABLE_SCRIPT_AUTO_ADD_CLASS") {
            enableScriptAutoClass = (value != "true");
        } else if (key == "DISABLE_SCRIPT_AUTO_ADD_ID") {
            enableScriptAutoId = (value != "true");
        } else if (key == "INDEX_INITIAL_COUNT") {
            try {
                int count = std::stoi(value);
                initialClassIndex = count;
                initialIdIndex = count;
            } catch (const std::exception&) {
                // 忽略无效配置
            }
        } else if (key == "AUTO_CLASS_PREFIX") {
            classPrefix = value;
        } else if (key == "AUTO_ID_PREFIX") {
            idPrefix = value;
        }
    }
}
