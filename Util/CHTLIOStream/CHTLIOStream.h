...
public:
     ...
     * @param encoding 编码类型
     */
    explicit CHTLIOStream(StreamMode mode = StreamMode::READ, Encoding encoding = Encoding::UTF8);
    
    /**
     * @brief 虚析构函数
     */
    virtual ~CHTLIOStream();
     ...
     * @return CHTL内容字符串
     */
    virtual std::string readCHTL();
    
    /**
     * @brief 写入CHTL内容
     * @param content CHTL内容
     * @return 是否成功写入
     */
    virtual bool writeCHTL(const std::string& content);
 ...
 * 专门用于处理CHTL模板文件
 */
class CHTLTemplateIOStream : public CHTLIOStream {
public:
    /**
     * @brief 构造函数
     * @param mode 流模式
     * @param encoding 编码类型
     */
    explicit CHTLTemplateIOStream(StreamMode mode = StreamMode::READ, Encoding encoding = Encoding::UTF8);
    
    /**
     * @brief 读取模板内容
     * @return 模板内容
     */
    virtual std::string readTemplate();
     ...
     * @return 是否成功写入
     */
    virtual bool writeTemplate(const std::string& templateContent);
     ...
     * @param name 模板名称
     */
    virtual void setTemplateName(const std::string& name);

private:
    std::string templateName_;          // 模板名称
};
 ...
 * 专门用于处理CHTL配置文件
 */
class CHTLConfigIOStream : public CHTLIOStream {
public:
    /**
     * @brief 构造函数
     * @param mode 流模式
     * @param encoding 编码类型
     */
    explicit CHTLConfigIOStream(StreamMode mode = StreamMode::READ, Encoding encoding = Encoding::UTF8);
